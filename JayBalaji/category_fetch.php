...
foreach($result as $row)
{
    ...
    */
    if($row['category_status'] == 'active')
    {
        $status = '<button type="button" name="status" id="'.$row["category_id"].'" class="btn btn-success btn-xs delete">active</button>';
    }
    else
    {
        $status = '<button type="button" name="status" id="'.$row["category_id"].'" class="btn btn-danger btn-xs delete">inactive</button>';
    }
	$sub_array = array();
